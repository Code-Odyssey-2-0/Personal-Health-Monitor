...
public class EditProfileDoctorActivity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        /*
        currentUserImg = FirEebaseDatabase.getInstance().getReference("DoctorProfile").child("1590965871687");
        Glide.with(this)
                .load(currentUserImg)
                .into(profileImage);
                   */
    ...
    private void uploadProfileImage() {
        ...
        if (uriImage != null) {
            ...
            storageReference.putFile(uriImage).continueWithTask(new Continuation<UploadTask.TaskSnapshot, Task<Uri>>() {
                @Override
                public Task<Uri> then(@Nopackage com.ensias.healthcareapp;
                ...
                }nNull Task<UploadTask.TaskSnapshot> task) throws Exception {
                    if (!task.isSuccessful()) {
                        throw task.getException();
                    }
                    return pStorageRef.getDownloadUrl();
